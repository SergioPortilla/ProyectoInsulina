...
const OrdersPage = () => {
  ...
  useEffect(() => {
    (async () => {
      const orderList: CreateOrder[] = await GetOrders(localValue as string);
  ...
  const deleteOrderForm = async (id: string) => {
    ...
    await DeleteOrders(localValue as string, algo);
    setLoading(!loading);
  };
  ...
  const updateOrderForm = async (orderUpdated: CreateOrder) => await UpdateOrders(localValue as string, orderUpdated);

  // No Any (Este lo dejamos pasar porque el tipado estaba complejo pero evitar)
  const orderSelectedChangeValue = (event: { target: { name: string; value: string } }) =>
    setOrderSelected({
      ...orderSelected,
  ...
  return (
    <BoxPermissions className="OrderContainer">
                  ...
                  indexOrder !== index ? (
                    <tr key={index}>
                      ...
                      <TituloUser className="OrdersForm">
                        <StatusComponent background={STATES[order.state as ActiveStates].background}>{STATES[order.state as ActiveStates].name}</StatusComponent>
                      </TituloUser>
                      <td>
                        {permissionsValue.includes(Permissions.read) && permissionsValue.includes(Permissions.delete) && (
                          ...
                          <ButtonForm className="botonDelete" onClick={() => deleteOrderForm(order.id as string)}>
      ...
      <ImgToDoCreationPage className="maproute" src="../assets/images/ilustrations/map-route.svg" alt="MapRoute" />
