...
export const LoginUser: (user: LogIn) => Promise<string | number> = (user: LogIn) => {
  ...
  return (
    axios
      .post<LoginResponseProps>(`/users/login`, body)
...
export const GetOrders: (token: string) => Promise<CreateOrder[]> = (token: string) => {
  ...
  return axios.get('/orders', config(token)).then((response) => response.data.data);
};
...
export const CreateOrders = (token: string, newOrder: CreateOrder) => {
  ...
  return axios.post<response, AxiosResponse<response>, CreateOrder>('/orders', newOrder, config(token)).then((response) => response.data.notification.description);
};

// DELETE ORDER REQUEST -- VERIFICADO

export const DeleteOrders = (token: string, deleteOrder: CreateOrder) => {
  return axios.put('/orders/delete', deleteOrder, config(token)).then((response) => response.data.notification.description);
};

// UPDATE ORDER REQUEST -- VERIFICADO

export const UpdateOrders = (token: string, updateOrder: CreateOrder) => {
  return axios.put('/orders/update', updateOrder, config(token)).then((response) => response.data.notification.description);
};
